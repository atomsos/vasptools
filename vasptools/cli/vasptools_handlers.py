...
"""
def _vasptools_cli_list_basis_sets(args):
...
def _vasptools_cli_list_ref_formats(args):
...
def _vasptools_cli_list_roles(args):
...
def _vasptools_cli_get_refs(args):
...
def _vasptools_cli_get_info(args):
    ...
    version_lines = format_columns([(k, compact_elements(v['elements']),
                                     v['revdesc']) for k, v in ver.items()], '    ')
    ret.extend(version_lines)

    return '\n'.join(ret)


def _vasptools_cli_get_notes(args):
    '''Handles the get-notes subcommand'''
    return api.get_basis_notes(args.basis, args.data_dir)


def _vasptools_cli_get_family(args):
    '''Handles the get-family subcommand'''
    return api.get_basis_family(args.basis, args.data_dir)


def _vasptools_cli_get_versions(args):
    '''Handles the get-versions subcommand'''
    name = args.basis.lower()
    metadata = api.get_metadata(args.data_dir)
    if not name in metadata:
        raise KeyError(
            "Basis set {} does not exist. For a complete list of basis sets, \
             use the 'list-basis-sets' command".format(name))
...
def _vasptools_cli_get_family_notes(args):
    '''Handles the get-family-notes subcommand'''
    return api.get_family_notes(args.family, args.data_dir)


def _vasptools_cli_create_bundle(args):
    '''Handles the create-bundle subcommand'''
    bundle.create_bundle(args.bundle_file, args.fmt, args.reffmt,
                         args.archive_type, args.data_dir)
...
def vasptools_cli_handle_subcmd(args):
    '''Handle subcmd'''
    handler_map = {
        'list-formats': _vasptools_cli_list_formats,
        'list-ref-formats': _vasptools_cli_list_ref_formats,
        'list-roles': _vasptools_cli_list_roles,
        'get-data-dir': _vasptools_cli_get_data_dir,
        'list-basis-sets': _vasptools_cli_list_basis_sets,
        'list-families': _vasptools_cli_list_families,
        'lookup-by-role': _vasptools_cli_lookup_by_role,
        'get-basis': _vasptools_cli_get_basis,
        'get-refs': _vasptools_cli_get_refs,
        'get-info': _vasptools_cli_get_info,
        'get-notes': _vasptools_cli_get_notes,
        'get-family': _vasptools_cli_get_family,
        'get-versions': _vasptools_cli_get_versions,
        'get-family-notes': _vasptools_cli_get_family_notes,
        # 'create-bundle': _vasptools_cli_create_bundle
    }

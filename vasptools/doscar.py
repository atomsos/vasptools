        ...
        for atomi in select_atoms:
            for spin_type in args.spin_type:
                ...
                label = label_fmt.format(
                    dos_type, spin_type, atomi % len(symbols), symbols[atomi])
                x = sdict['{0}/{1}/energy'.format(basepath, spin_type)]
                if x.ndim > 1:
                    x = x[atomi]
                if spin_type in ['spin1', 'spin2']:
                    y = sdict['{0}/{1}/{2}'.format(basepath,
                                                   spin_type, angular)]
                else:
...
def cli_add_parser(subparsers):
    ...
    subp.add_argument('--outputdir', nargs=1, metavar='PATH',
                      default='.', help='output directory')
    subp.add_argument('--title', metavar='string', help='title of picture')
    subp.add_argument('--atoms', nargs='*', help='generate picture')
    subp.add_argument('--xlim', nargs=2, metavar='float',
                      help='xlimit of picture')
    subp.add_argument('--ylim', nargs=2, metavar='float',
                      help='ylimit of picture')
    subp.add_argument('-a', '--angular_momentum', nargs='*',
                      help='selected angular momentum, supports:{0}'.format(SUPPORTED_ANGULAR_MOMENTUM))
    subp.add_argument('--spin_type', nargs='*',
                      metavar='float', help='ylimit of picture')
    subp.add_argument('--xregion', nargs=2, metavar='float',
                      help='x region of system')
    subp.add_argument('--yregion', nargs=2, metavar='float',
                      help='y region of system')
    subp.add_argument('--zregion', nargs=2, metavar='float',
                      help='z region of system')

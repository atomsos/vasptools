...
def get_potcar_content(pp_names=None, pp_type=DEFAULT_PP_TYPE):
    ...
    assert pp_type in utils.VALID_PP_TYPES, '{0} should belongs {1}'.format(
        pp_type, utils.VALID_PP_TYPES)
...
def get_avail_pot(pp_names=None, pp_type=DEFAULT_PP_TYPE, preview=False):
    ...
    def list_element_pot(element):
        reg_path = os.path.join(
            utils.VASPPOT_PATH, pp_type, element+'*', POTCAR_STRING+'*')
...
def test(test_dir=None):
    ...
    test of potcar
    """
    test_dir = test_dir or os.path.join(os.path.dirname(
        os.path.abspath(__file__)), '..', 'test_dir')
...
def cli_add_parser(subparsers):
    ...
    subp.add_argument('-i', '--input', metavar='PATH',
                      help='Generate POTCAR with POSCAR')
    subp.add_argument('-d', '--dirname', metavar='PATH',
                      default='.', help='directory to generate file')
    subp.add_argument('-p', '--ptype', default=DEFAULT_PP_TYPE,
                      help='potcar type, avail: {0}'.format(utils.VALID_PP_TYPES))

...
def get_potcar_content(pp_names=None, pp_type=DEFAULT_PP_TYPE):
    ...
    assert pp_type in utils.VALID_PP_TYPES, \
        '{0} should belongs {1}'.format(pp_type, utils.VALID_PP_TYPES)
...
def get_avail_pot(pp_names=None, pp_type=DEFAULT_PP_TYPE, preview=False):
    ...
    def list_element_pot(element):
        """
        list available pot for element
        """
        reg_path = os.path.join(utils.VASPPOT_PATH,
                                pp_type, element+'*', POTCAR_STRING+'*')
...
def cli_add_parser(subparsers):
    ...
    potcar add_parser
    """
    subp = subparsers.add_parser(MOD_NAME, help=POTCAR_STRING)
    subp.add_argument('-i', '--input', metavar='PATH',
                      help='Generate POTCAR with POSCAR')
    subp.add_argument('-d', '--dirname', metavar='PATH', default='.',
                      help='directory to generate file')
